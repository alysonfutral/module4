...
class Team(IdentifiedObject):
    ...
    def add_member(self, member):
        for the_member in self._members: # check if member is in list of members
            # Raise DuplicateEmail if a team member is added to a
            # team that already has a member with that e-mail address.
            if the_member.email.lower() == member.email.lower(): # case sensitive
                raise DuplicateEmail(member.email)# if the team member is in the list, leave
            # check if oid is already in collection
            if the_member.oid == member.oid:
